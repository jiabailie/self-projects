...
int _clusterMRF(int num, int l_dis, int h_dis, int* l_num, int* h_num, IMAGE_L_MRF* l_mrf, IMAGE_H_MRF* h_mrf, L_cluster** l_cluster, H_cluster** h_cluster)
{
	...
	/* add 0 to the count struct */
	iCount_l = 1;
	l_count[0].l = 0;
	l_count[0].c = 2147483647;
...
int _clusterLMRF(int dis, int num, int* cnum, L_count* l_count, L_cluster* l_cluster)
{
	...
	for(i = 0; i < num; i++)
	{
		...
		if(used[i])
		{
			...
			l_cluster[(*cnum)].subNum = 1;
			l_cluster[(*cnum)].sub[0] = l_count[i].l;
...
int _clusterHMRF(int dis, int num, int* cnum, H_count* h_count, H_cluster* h_cluster)
{
	...
	for(i = 0; i < num; i++)
	{
		...
		if(used[i])
		{
			...
			h_cluster[(*cnum)].subNum = 1;
			h_cluster[(*cnum)].sub[0].lb = h_count[i].lb;
			h_cluster[(*cnum)].sub[0].lt = h_count[i].lt;
			h_cluster[(*cnum)].sub[0].rb = h_count[i].rb;
			h_cluster[(*cnum)].sub[0].rt = h_count[i].rt;
			if(i + 1 < num)
			{
				for(j = i + 1; j < num; j++)
				{
